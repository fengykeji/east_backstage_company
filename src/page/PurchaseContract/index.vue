  ...
  <div class='purchaseContract project'>
    ...
    <div class="printExcel">
      <div class='contract'>
        ...
        <div>委托方（甲方）：
          <span class="underline"> {{printList.user_name}} </span> 身份证号码：
          <span class="underline"> {{printList.card_id}} </span>
        </div>
        <div>受托方（乙方）：
          <span class="underline"> {{printList.agent_name}} </span> 身份证号码：
          <span class="underline"> {{printList.agent_tel}} </span>
        </div>
        <div>根据《中华人民共和国合同法》等有关法律、法规规定，甲、乙双方在协商一致的基础上，就甲方委托乙方代为购买房屋事宜，达成如下协议：</div>
        <div>一、委托事项：</div>
        <div>1、甲方委托乙方以乙方名义购买房屋，房价款由甲方实际支付。乙方以自己的名义与
          <span class="underline"> {{printList.user_name}} </span> 签订《房屋买卖合同》，并负责办理登记在乙方名义下的房屋产权证等相关手续。</div>
        <div>2、委托代购房屋的基本情况：</div>
        <div>2.1代购房屋位于
          <span class="underline"> {{printList.project_city_name}} </span> 市
          <span class="underline"> {{printList.project_district_name}} </span>区
          <span class="underline"> {{printList.project_address}} </span>路/街道
          <span class="underline"> {{printList.project_name}} </span>花园/大厦
          <span class="underline"> {{printList.build_name}} </span>栋
          <span class="underline"> {{printList.unit_name}} </span>单元
          <span class="underline"> {{printList.house_name}} </span>号
        </div>
        <div>
          <span class="underline"> {{printList.permit_code}} </span>房屋所有权证号为
          <span class="underline"> {{printList.permit_code}} </span>；国土使用证号为
          <span class="underline"> {{printList.land_use_permit_code}} </span>。（以房产证相关有效法律文件为准）</div>
        <div>2.2代购房屋面积约为
        ...
        <div>二、房屋价款的支付方式：</div>
        <div>1、甲、乙双方确认：本协议签订之日，乙方以自己的名义向银行申请按揭贷款，签订《借款合同》。按揭贷款人民币
          <span class="underline"> {{printList.a}} </span>元，元，用于一次性支付代购房屋的购房款；</div>
        ...
        <div>1、乙方应保证按照本协议第一条确认的内容与
          <span class="underline"> {{printList.user_name}} </span>签订《房屋买卖合同》；</div>
        ...
        <div>1、因甲方原因，导致本协议无法履行，乙方有权要求甲方向乙方支付违约金人民币
          <span class="underline"> {{printList.break_money}} </span>元。</div>
        ...
        <div class='row_center'>
          <span class='display'>
            甲 方：
            <span class="underline"> {{printList.user_name}} </span>
            <div class='margin-30px'>（签字或盖章）：</div>
            <div>
              <span class="underline"> {{printList.day_date}} </span>
              <!-- <span class="underline"> {{printList.name}} </span>月
              <span class="underline"> {{printList.name}} </span>日 -->
            </div>
          </span>
          <span class='display'>
            乙 方：
            <span class="underline"> {{printList.agent_name}} </span>
            <div class='margin-30px'>（签字或盖章）：</div>
            <div>
              <span class="underline"> {{printList.day_date}} </span>
              <!-- <span class="underline"> {{printList.name}} </span>年
              <span class="underline"> {{printList.name}} </span>月
              <span class="underline"> {{printList.name}} </span>日 -->
            </div>
          </span>
        ...
        <div class='margin-t-20px'>合同签订地：
          <span class="underline"> {{printList.store_name}} </span>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    //打印
    async printing(row, type) {
      let res = await this.api.print({ sub_id: row.sub_id });
      if (res.code == 200) {
